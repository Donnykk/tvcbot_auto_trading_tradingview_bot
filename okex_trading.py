...
if os.path.exists("./config.json"):
    ...
    config["account"]["enable_proxies"] = (
        config["account"]["enable_proxies"].lower() == "true"
    )
    config["trading"]["enable_stop_loss"] = (
        config["trading"]["enable_stop_loss"].lower() == "true"
    )
    config["trading"]["enable_stop_gain"] = (
        config["trading"]["enable_stop_gain"].lower() == "true"
    )
else:
...
def sltpThread(oid, side, symbol, sz, tdMode, config):
    ...
    if config["trading"]["enable_stop_loss"]:
        privatePostTradeOrderAlgoParams["slTriggerPx"] = config["trading"][
            "stop_loss_trigger_price"
        ]
        privatePostTradeOrderAlgoParams["slOrdPx"] = config["trading"][
            "stop_loss_order_price"
        ]
    if config["trading"]["enable_stop_gain"]:
        privatePostTradeOrderAlgoParams["tpTriggerPx"] = config["trading"][
            "stop_gain_trigger_price"
        ]
        privatePostTradeOrderAlgoParams["tpOrdPx"] = config["trading"][
            "stop_gain_order_price"
        ]
    while True:
        try:
            ...
            if privateGetTradeOrderRes["data"][0]["state"] == "filled":
                ...
                slTriggerPx = (
                    1
                    + direction
                    * float(config["trading"]["stop_loss_trigger_price"])
                    * 0.01
                ) * avgPx
                tpOrdPx = (
                    1
                    + direction
                    * float(config["trading"]["stop_gain_order_price"])
                    * 0.01
                ) * avgPx
                tpTriggerPx = (
                    1
                    - direction
                    * float(config["trading"]["stop_gain_trigger_price"])
                    * 0.01
                ) * avgPx
                slOrdPx = (
                    1
                    - direction
                    * float(config["trading"]["stop_loss_order_price"])
                    * 0.01
                ) * avgPx
                ...
                if (
                    "code" in privatePostTradeOrderAlgoRes
                    and privatePostTradeOrderAlgoRes["code"] == "0"
                ):
...
def closeAllPosition(_symbol, _tdMode):
    try:
        res = exchange.privatePostTradeClosePosition(
            params={"instId": _symbol, "mgnMode": _tdMode, "posSide": "short"}
        )
...
def createOrder(
    ...
    stop_gain_trigger_price=0,
    stop_gain_order_price=0,
):
    try:
        # 挂单
        balance = exchange.fetch_balance()
        usdt_balance = balance["USDT"]["free"]
        logging.info("balance:" + str(usdt_balance))
        posSide = ""
        logging.info("side:" + _side)
        if _side == "buy":
            posSide = "long"
        else:
            posSide = "short"
        ...
        if (
            config["trading"]["enable_stop_loss"]
            or config["trading"]["enable_stop_gain"]
        ):
            try:
...
def amountConvertToSZ(_symbol, _amount, _price, _ordType):
    ...
    if _symbolSplit[1] == "USD":
        ...
        if _ordType.upper() == "MARKET":
            _price = exchange.publicGetPublicMarkPrice(
                params={
                    "instId": _symbol,
                    "instType": ("SWAP" if isSwap else "FUTURES"),
                }
            )["data"][0]["markPx"]
        sz = sz * float(_price)
    return int(sz)
...
def order():
    ...
    if _params["side"].lower() in ["buy", "sell"]:
        ...
        if sz < 1:
            ...
            ret["createOrderRes"], ret["msg"] = createOrder(
                _params["symbol"],
                sz,
                _params["price"],
                _params["side"],
